...
class RobertaEmbeddingModel(BaseEmbeddingModel):
    ...
    def encode(self, texts: List[str], max_length: int = 512):
        with torch.no_grad():
            inputs = self.tokenizer(texts, padding=True, truncation=True, max_length=max_length, return_tensors='pt').to(self.embedding_model.device)
            outputs = self.embedding_model(**inputs)
            # For RoBERTa, typically use the last hidden state and perform pooling
    ...
    def batch_encode(self, texts: List[str], **kwargs) -> None:
        ...
        logger.debug(f"Calling {self.__class__.__name__} with batch size {batch_size}")
        if len(texts) <= batch_size:
            results = self.encode(texts, max_length=max_length)
        else:
            pbar = tqdm(total=len(texts), desc="Batch Encoding")
            results = []
            for i in range(0, len(texts), batch_size):
                results.append(self.encode(texts[i:i + batch_size], max_length=max_length))
